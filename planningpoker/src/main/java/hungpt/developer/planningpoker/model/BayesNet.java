...
public class BayesNet {
	...
	public BayesNet(List<Resource> resources, List<Task> tasks) {
		...
		this.resourceNet = new ArrayList<Network>();
	}
	...
	public BayesNet(List<Resource> resources, int totalTasks) {
        ...
        this.resourceNet = new ArrayList<>();
    }

    public BayesNet(Resource resource, List<Task> tasks) {
	...
	public void construct() throws IOException {
		...
		for (int i = 0; i < numResources; i++) {
			...
			System.out.println("num tasks" + String.valueOf(numTasks));
	...
	public double[][] infer() throws InconsistentEvidenceException {
		...
		for (int i = 0; i < numResources; i++) {
			...
			if (numTasks > 1) {
				...
				Table queryLastTask = new Table(neti.getNodes().get("task0"));
				State stateTrue = neti.getNodes().get("task0").getVariables().get(0).getStates().get("True");
				inference.getQueryDistributions().add(queryLastTask);
				inference.query(queryOptions, queryOutput);
				result[i][0] = queryLastTask.get(stateTrue);
			}
		}
		return result;
