...
public class Panel extends JPanel {
	...
	public void createButton(int[] arrSL, List<String> listNameTask, int width, int height, double[] arrProb, int nameResource)
		...
		int currentTime = 0;
		System.out.println("list name task size" + listNameTask.size());
		System.out.println("arrSL" + arrSL.length);
		System.out.println("arrProb" + arrProb.length);
		//for (int i = 0; i < arrSL.length; i++) {
		for (int i = 0; i < listNameTask.size(); i++) {
			if (arrSL[i] != 0) {
//System.out.println(listNameTask.get(i) + "  " +  arrSL[i] + "  " + arrProb[i]);
