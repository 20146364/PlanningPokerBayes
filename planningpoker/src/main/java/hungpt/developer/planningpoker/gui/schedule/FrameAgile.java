...
public class FrameAgile extends JFrame {
     ...
     */
    public FrameAgile(BayesNet net, final int[][] arrTimeExcute, final List<List<String>> scheduleNameTask)
        ...
        JButton save = new JButton("Lưu kết quả");
        save.addActionListener(new ActionSaveData(scheduleNameTask, dataProb));
        panelArea.add(okBtn);
    ...
    public static double caculateProbTotal(double[][] dataProb) throws InconsistentEvidenceException {
        ...
        for (int i = 0; i < nResources; i++) {
            ...
            totalProb += nTasksPerRes > 0 ? dataProb[i][nTasksPerRes - 1] : 0;

        }
        return totalProb / nResources;
    }
}
...
final  class ActionSaveData implements ActionListener {
    ...
    private double[][] prob;
    public ActionSaveData(List<List<String>> scheduleNameTask, double[][] prob){
